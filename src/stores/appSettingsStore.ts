...
/**
 * Persiste as premissas de "Consumo e bateria" e "Carregamento" do Menu
 * Lateral no Supabase (tabela app_settings, linha unica id='default'),
 * mesmo padrao de debounce (500ms) e mutacao ao vivo do vehicleProfileStore.
 */
...
export const useAppSettingsStore = create<AppSettingsStore>((set, get) => ({
  ...
  load: async () => {
    const { data, error } = await client
      .from('app_settings')
      .select('reserve_soc_pct, energy_tariff_brl_kwh, gas_price_brl_l, gas_km_per_l')
      .eq('id', SETTINGS_ID)
      .maybeSingle();
    if (error || !data) {
